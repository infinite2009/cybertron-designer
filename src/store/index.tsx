...
const reducer = (state: IEditorProps, action: ActionType) => {
    ...
    const currentComponent = state.components.filter((item) => item.id === state.currentElement)[0]
    ...
    switch (action.type) {
        ...
        case actionTypes.DELETECOMPONENT:
            ...
            if (currentComponent) {
                components = components.filter((item) => item.id !== state.currentElement)
        ...
        case actionTypes.MOVECOMPONENT:
            if (currentComponent) {
                ...
                const { direction, amount } = value as { direction: MoveDirection, amount: number }
                let key = '', newValue = ''
                switch (direction) {
                    case 'Up': {
                        key = "top"
