...
const Index: React.FC<IProps> = (props) => {
    ...
    return (
        <div>
            {finalProps && Object.keys(finalProps).map((key: string) => {
                const values = finalProps[key];
                const { component: Component, subComponent: SubComponent, options, valueProp, value, extraProps, events } = values;
                const domProps = {
                    [valueProp]: value, ...extraProps,
                    ...events,
                };
                return (
                    <Fragment key={key}>
                            ...
                            <Row className={styles['prop-item']}>
                                ...
                                <Component {...domProps}>
                                    {options && options.length ? options.map((option) => {
                                        return (
                                            <SubComponent value={option.value} key={option.value}>
                                                {option.text}
                                            </SubComponent>
                                        );
                                    })
